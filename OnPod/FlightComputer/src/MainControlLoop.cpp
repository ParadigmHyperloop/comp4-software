 ...
 */
bool nominalStates(BreakNodeStates bnState, BreakNodeStates bnAccepted[],int bnAcceptedSize){
...
bool armedToFlight(PodValues* podVals){
	...
	if(terminalState == terminalFlight /
			& nodeState == bnFlight ){
		podVals->terminalCommand = terminalNone;
...
int runControlLoop(PodValues* podVals){
	...
	switch(podState){
		...
		case standby:{
			...
			if( checkFlags(podVals) & nominalStates(podVals->breakNodeS, bnAccepted, 2)){
		...
		case armed:{
			...
			if(checkFlags(podVals & nominalStates(podVals->breakNodeS, bnAccepted, 1) )){
