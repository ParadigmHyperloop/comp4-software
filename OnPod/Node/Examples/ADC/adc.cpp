...
uint16_t ADS7953::readSingleChannel(uint8_t iChannel) {
    uint16_t iReadCommand = MANUAL_READ;
    // insert the channel address into bits 10-7 of the command
    iReadCommand |= (iChannel << 7);
    // data results are from two data frames ago;
    // send two data requests before we start to read
    transfer(iReadCommand);
    transfer(iReadCommand);
    return transfer(iReadCommand) & 0x0FFF;
}
...
void ADS7953::readActiveChannels() {
    ...
    for(uint8_t i = 0; i < iNumChannels; i++) {
        uint16_t iConversionData = transfer(AUTO_READ_NEXT);
