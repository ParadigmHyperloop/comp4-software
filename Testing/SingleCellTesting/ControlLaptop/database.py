...
class Database:
    def __init__(self):
        ...
        self.client.create_database("single_cell_db")
        self.client.switch_database("single_cell_db")

    # find the next available number to identify a test
    def find_next_test_number(self):
        results = self.client.query(
            'SELECT test_number FROM "single_cell_db".."sensor_data"'
        )
        try:
    ...
    def export_test(self, test_number):
        results = self.client.query(
            f'SELECT * FROM "single_cell_db".."sensor_data" WHERE test_number = {test_number}'
        )
        try:
